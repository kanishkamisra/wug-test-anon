...
# Fork of core/train/embed_train.py (kept unmodified): only run_agreement_eval()'s
# dev-set metric differs (mean logprob margin instead of win-rate accuracy) — see
# eacl-rebuttal/README.md. Path shim below lets this run from any cwd.
import pathlib
...
def main():
    ...
    def run_agreement_eval():
        # lm.sequence_score defaults to mean-logprob-per-token (minicons'
        # VLMScorer.sequence_score reduction=lambda x: x.mean(0).item()), so gs/bs
        # are already avg-logprob-per-token for the good/bad sentence. The dev
        # metric here is the mean margin diffs.mean() == avg[LP(good)] - avg[LP(bad)],
        # i.e. for the noun-vs-verb dev set:
        #   LP(wug as noun) + LP(wugs as noun) - LP(wug as verb) - LP(wugs as verb)
        # (up to averaging over pairs instead of summing), NOT a binarized win-rate.
        # "acc"/"acc_mask" names are kept only so the rest of the script (early
        # stopping, CSV schemas) needs zero other changes; acc_mask is still the
        # binarized win/loss mask, logged per-item but no longer the selection metric.
        ...
        acc = (diffs > 0).float()
        return {
            "overall_acc": diffs.mean().item(),
            "sing_acc": diffs[singular_eval_idx].mean().item(),
            "plur_acc": diffs[plural_eval_idx].mean().item(),
            "diffs": diffs.cpu(),
            "good_scores": gs.cpu(),
            "bad_scores": bs.cpu(),
            "acc_mask": acc.cpu(),
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"  Eval[margin]: overall={ev['overall_acc']:.3f} sing={ev['sing_acc']:.3f} plur={ev['plur_acc']:.3f}")
